...
#      @new_deals = get_deal_status_count([1], user, associated_by).count
      unless (params[:q].present? && params[:y].present?)
         @filter_msg="Filtered #{@total_new_deals} deals #{associated_by} #{name} from #{count_total} new deals."
      else
         @filter_msg="Filtered <span id='count_deal1' style='display:none;'></span> deals #{associated_by} #{name} from #{count_total} new deals."
      end
    end
...
#        contact = Contact.find params[:deal][:contact_id]
      ...
      if contact.present?
        ...
        deal.save
	      flash[:notice] = "Deal has been saved successfully."
  ...
  def update
    ...
    if !request.xhr?
      if params[:quick_edit].present?
       ...
       redirect_to (session[:prev_page].nil? || session[:prev_page].include?('edit_individual_contact') || session[:prev_page].include?('edit_company_contact')) ? deals_path : session[:prev_page]
       session[:prev_page] = nil
      end
    else
      render :text=>"success"
...
# #    contact.save
  ...
  def destroy  
    ...
    @deal = Deal.find(params[:id])
    return_url = @deal.is_remote? ? deals_path(:type=>"un_assigned") : deals_path()
